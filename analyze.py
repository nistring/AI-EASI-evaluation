...
if __name__ == "__main__":
    ...
    lm = sns.regplot(data=df, x="True", y="Pred.", scatter_kws={"s": 2.}, robust=True, truncate=False)
    ...
    lm.set(xlim=(0., None), ylim=(0., None))
    r, p = spearmanr(gt_kappa, kappa, alternative="greater")
    ...
    lm = sns.lmplot(
        data=df, x="True", y="Pred.", col="Sign", hue="Sign", x_estimator=np.mean, x_ci="sd", robust=True, truncate=False
    )
    lm.set(xlim=(-0.05, 3.05), ylim=(-0.05, 3.05))
    lm.figure.suptitle(f"{args.exp_name}'s severity score")
    axes = lm.axes
    ...
    lm = sns.lmplot(data=df, x="True", y="Pred.", col="Sign", hue="Sign", scatter_kws={"s": 2.}, legend=False, robust=True, truncate=False)
    lm.set(xlim=(-0.5, 18.), ylim=(-0.5, 18.))
    ...
    g.figure.suptitle(f"Spearman's rho on std of {args.exp_name}'s EASI score by sign")
